...
//  Instragram-Clone
//
//  Created by Chanoknun Choosaksilp on 25/10/2564 BE.
//
...
class NotificationsViewModel: ObservableObject {
    ...
    func fetchNotifications(){
        ...
        Firestore.firestore().collection("notifications").document(userID).collection("user-notifications").order(by: "timestamp", descending: true).getDocuments{
            (snap,error) in
            if let error = error {
