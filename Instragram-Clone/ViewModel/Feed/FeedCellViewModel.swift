...
class FeedCellViewModel: ObservableObject {
    ...
    func like() {
        ...
        Firestore.firestore().collection("posts").document(postID).collection("post-likes").document(userID).setData([:]) { error in
            ...
            Firestore.firestore().collection("users").document(userID).collection("user-likes").document(postID).setData([:]) { error in
                ...
                Firestore.firestore().collection("posts").document(postID).updateData(["likes": self.post.likes + 1]) { error in
                    if let error = error {
                        print(error.localizedDescription)
                        return
                    }
    ...
    func checkLike(){
        ...
        Firestore.firestore().collection("posts").document(postID).collection("post-likes").document(userID).getDocument{
            (snap,error) in
            if let error = error {
                print(error.localizedDescription)
                return
